...
{#snippet inline(parts: Inline[])}
	{#each parts as part, i (i)}
		...
		{:else if part.kind === 'link'}
			...
			<button type="button" class="break-all underline" onclick={() => openUrl(part.href).catch(() => {})}>
				{part.text}
			</button>
		{:else}
			{part.text}
...
<div class={cls}>
	{#each blocks as block, i (i)}
		...
		{:else if block.kind === 'list'}
			<svelte:element this={block.ordered ? 'ol' : 'ul'} class={['mt-1.5 pl-4 first:mt-0', block.ordered ? 'list-decimal' : 'list-disc']}>
				{#each block.items as item, j (j)}
					<li class="mt-0.5">{@render inline(item)}</li>
				{/each}
			</svelte:element>
		{:else}
			<pre class="mt-2 overflow-x-auto rounded-lg bg-neutral-300 p-2 font-mono text-[0.9em]">{block.text}</pre>
		{/if}
	{/each}
</div>
