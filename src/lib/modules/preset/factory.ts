...
function make<K extends PresetKind>(
	kind: K,
	name: string,
	data: Preset<K>['data']
): Preset<K> {
	return { id: `builtin:${kind}:${name}`, kind, name, data, createdAt: 0, builtin: true, version: PRESET_VERSION };
}

// EQ bands are the ten ISO octave centres: 31.5 63 125 250 500 1k 2k 4k 8k 16k.
export const FACTORY_PRESETS: Preset[] = [
	make('compressor', 'Vocal', { thresholdDb: -18, ratio: 3, attackMs: 8, releaseMs: 120, kneeDb: 6, makeupDb: 3 }),
	make('compressor', 'Podcast Leveler', { thresholdDb: -24, ratio: 4, attackMs: 15, releaseMs: 200, kneeDb: 8, makeupDb: 5 }),
	make('compressor', 'Drum Bus', { thresholdDb: -12, ratio: 4, attackMs: 3, releaseMs: 80, kneeDb: 3, makeupDb: 2 }),
	make('compressor', 'Master Glue', { thresholdDb: -10, ratio: 2, attackMs: 30, releaseMs: 250, kneeDb: 9, makeupDb: 1 }),
	make('compressor', 'Peak Tamer', { thresholdDb: -6, ratio: 10, attackMs: 1, releaseMs: 50, kneeDb: 0, makeupDb: 0 }),

	make('noiseGate', 'Vocal Cleanup', { thresholdDb: -45, rangeDb: -30, attackMs: 1, holdMs: 60, releaseMs: 120 }),
	make('noiseGate', 'Room Tone Kill', { thresholdDb: -55, rangeDb: -20, attackMs: 5, holdMs: 150, releaseMs: 250 }),
	make('noiseGate', 'Drum Tight', { thresholdDb: -30, rangeDb: -40, attackMs: 0.5, holdMs: 30, releaseMs: 60 }),
	...
	make('noiseSuppressor', 'Speech Light', { attenuationLimitDb: 12, postFilterBeta: 0, minThreshDb: -10, maxErbThreshDb: 30, maxDfThreshDb: 20 }),
	make('noiseSuppressor', 'Speech Strong', { attenuationLimitDb: 40, postFilterBeta: 0.02, minThreshDb: -5, maxErbThreshDb: 25, maxDfThreshDb: 15 }),
	make('noiseSuppressor', 'Max Suppression', { attenuationLimitDb: 100, postFilterBeta: 0.05, minThreshDb: 0, maxErbThreshDb: 20, maxDfThreshDb: 10 })
];
